...
(defun k-mers (str k)
  ...
  (let ((suf-arr (make-suffix-array str))
        ...
        (curr-freq 0))
    (dolist (i suf-arr)
      (when (>= (length (subseq str i)) k)      ; skip all elements with length <k
        (let ((new-k-mer (subseq str i (+ i k))))
          (cond
...
(defun min-lexicographic-rotation (str)
  ;; Determine its minimum lexicographic rotation
  ;; Sequence of lexicographic rotations for "aaba" is "abaa", "baaa", "aaab".
  ;; The smallest is "aaab"
  ;; in order to slolve this need to concatenate input string with itself, 
  ;; create suffix array and find the smallest one
  (let* ((str-concat (concatenate 'string str str))
         (sf-array (make-suffix-array str-concat))
         (str-lth (length str)))
    (dolist (i sf-array)
      (when (>= (length (subseq str-concat i)) str-lth)
	    (return-from min-lexicographic-rotation (subseq str-concat i (+ i str-lth)))))))
